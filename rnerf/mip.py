...
def expected_sin(x, x_var):
  ...
  y = jnp.exp(-0.5 * x_var) * math_utils.safe_sin(x)
  y_var = jnp.maximum(
      0, 0.5 * (1 - jnp.exp(-2 * x_var) * math_utils.safe_cos(2 * x)) - y**2)
  return y, y_var


def lift_gaussian(d, t_mean, t_var, r_var, diag, near):
  """Lift a Gaussian defined along a curved ray to 3D coordinates."""
  # NOTE(ipe): https://research.nvidia.com/publication/2021-08_Refraction-Ray-Cones
  t = jnp.concatenate([t_mean[:, 0:1] - near, t_mean[:, 1:] - t_mean[:, :-1]], axis=-1)[..., None]
  mean = jnp.cumsum(d * t, axis=1)
  ...
  if diag:
    ...
    null_outer = eye - d[..., :, None] * (d / d_mag_sq)
...
def conical_frustum_to_gaussian(d, t0, t1, base_radius, diag, near, stable=True):
  ...
  return lift_gaussian(d, t_mean, t_var, r_var, diag, near)


def cylinder_to_gaussian(d, t0, t1, radius, diag, near):
...
def cast_rays(t_vals, origins, directions, radii, ray_shape, near, diag=True):
  ...
  if ray_shape == 'cone':
    ...
    assert False
  means, covs = gaussian_fn(directions, t0, t1, radii, diag, near)
  means = means + origins[:, 0:1]
  return means, covs
